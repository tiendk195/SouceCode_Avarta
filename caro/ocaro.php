		...
		while($a<$socot){
			...
			while($b<$socot) {
				...
				if(in_array($a.'|'.$b, $data2)){echo '<img src="'.$data1[array_search($a.'|'.$b, $data2)].'.'.$b1.'">';}
					...
					if(mysql_result($result,0,'turn')==$datauser['id']) {echo '<a class="cr" href="#up" onclick="danhco('.$a.','.$b.')">';}
				...
				<img src="b.<?php echo $c1; ?>"> <?php } ?>
					...
					if(mysql_result($result,0,'turn')==$datauser['id']) {echo '<a href="#up" onclick="danhco('.$a.','.$b.')">';}
					echo '<img src="b.'.$c1.'">';
					if(mysql_result($result,0,'turn')==$datauser['id']) {echo '</a>';}
				}
				echo '</td>';
