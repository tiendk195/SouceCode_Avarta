...
if($_GET['id']){
	...
	if(mysql_numrows($result)>0) {
		...
		if($view!=1){
		if(mysql_result($result,0,'turn')==$datauser['id'] && $view!=1){echo '<font color="green"><b>Đến lượt của bạn';}
		if(mysql_result($result,0,'turn')==mysql_result($result,0,'p'.$o) && $view!=1 && mysql_result($result,0,'turn')!=0){echo '<font color="red"><b>Lượt của đối thủ:';}}
		else{
		echo '<font color="green"><b>Đến lượt của ';
		if(mysql_result($result,0,'turn')==mysql_result($result,0,'px')){echo '<a href="/member/'.mysql_result($result,0,"px").'.html"><b>'.mysql_result($result,0,"xname").'</b></a>';}
		else {echo '<a href="/member/'.mysql_result($result,0,"po").'.html"><b>'.mysql_result($result,0,"oname").'</b></a>';}
		}
		...
		$time='<font color="blue"> thời gian còn '.$phut.' phút '.$giay.' giây</font>.';
		} else {$time='<font color="red"> (hết giờ) </font.';}
		echo ' '.$time.'</b></font><br>';}
