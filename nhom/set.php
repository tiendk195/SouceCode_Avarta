...
if(isset($_POST['sub'])) {
if($ri == 2){
mysql_query("UPDATE `nhom_user` SET `rights`='0' WHERE `id`='$id' AND `user_id`='$user_id'");
mysql_query("UPDATE `nhom` SET `user_id`='$sid' WHERE `id`='$id'");
}
...
echo '<div class="list1"><b>Quyền hạn:</b><br/><form method="post"><input type="radio"'.($user['rights']==0 ? ' checked="checked"':'').' value="0" name="ri" /> <span class="xam">Thành viên</span><br/><input type="radio"'.($user['rights']==1 ? ' checked="checked"':'').' value="1" name="ri" /> <span class="xam">Quản trị viên</span><br/><input type="radio"'.($user['rights']==2 ? ' checked="checked"':'').' value="2" name="ri" /> <span class="xam">Chủ Nhóm</span><br/><input type="submit" name="sub" value="Lưu" /></div>';
break;
case 'duoi':
...
mysql_query("UPDATE `users` SET `icon`='0' WHERE `id`='$sid'");
