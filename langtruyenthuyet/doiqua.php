...
require('../incfiles/head.php');
echo'<div class="phdr">Đổi Điểm Tích Lũy</div>';
...
echo'<div class="omenu"><center>Bạn đang có <b> <font color="red">'.number_format($datauser['tichluy']).'</font></b> điểm tích lũy </center></div>';
...
switch($act) {
default:
$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `shoplangtt` "),0);
$req=mysql_query("SELECT * FROM `shoplangtt` ORDER BY `id` DESC LIMIT $start,$kmess");
...
$id=$_GET[id];
$item=mysql_fetch_array(mysql_query("SELECT * FROM `shoplangtt` WHERE `id`='".$id."'"));
$shop=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$item[idshop]."'"));
  $xyz=mysql_num_rows(mysql_query("SELECT * FROM `shoplangtt` WHERE `id`='".$id."'"));
...
@mysql_query("UPDATE `shoplangtt` SET
`diem`='".$_POST[tichluy]."',
...
echo 'Nhập điểm: <input type="number" name="tichluy" value="'.$item[diem].'"><br/>';
...
mysql_query("DELETE FROM `shoplangtt` WHERE `id`='".$id."'");
    header ('Location : doiqua.php ');

}
$checkit = mysql_result(mysql_query("select count(*) from `shoplangtt` where `id` = '".$id."'"),0);
if ($checkit == 0) {
...
mysql_query("INSERT INTO `shoplangtt` SET
`idshop`='".$vatpham."',
`diem`='".$diem."',
`timebd`='".$hsd."'");
...
$id = (int)$_GET['id'];
$check=mysql_num_rows(mysql_query("SELECT * FROM `shoplangtt` WHERE `id`='".$id."'"));
if ($check<1) {
header('Location: doiqua.php');
exit;
}
...
if($datauser['tichluy'] >= $item['diem']){
    
    if ($item['timebd'] !=0)
{
$timesd= $item['timebd']*24*3600+time();
}
mysql_query("UPDATE `users` SET `tichluy` = `tichluy` - '".$item['diem']."' WHERE `id` = '".$user_id."'");
