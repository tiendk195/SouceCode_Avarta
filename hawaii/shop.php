...
require('../incfiles/head.php');
echo'<div class="phdr">Shop Hawaii</div>';
echo'<div class="omenu">Bạn đang có '.$datauser['mattrang'].' <img src="/icon/damattrang.png"/> đá mặt trăng, mua tại đây <a href="muada.php">Shop đá mặt trăng </a></div>';
switch($act) {
default:
$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `hawaii` "),0);
$req=mysql_query("SELECT * FROM `hawaii` ORDER BY `id` DESC LIMIT $start,$kmess");
...
<font color="ff007f">Vật phẩm:</font>
          '.$shop[tenvatpham].' ('.($shop['timesudung'] ? thoigiantinh(floor($shop['timesudung'])).'' : 'Vĩnh viễn').')<br>
          <font color="ff007f">Giá bán:</font> '.number_format($res['mattrang']).' Đá mặt trăng </font><br>
...
if ($tong > $kmess){
echo '<div class="topmenu">' . functions::display_pagination('shop.php?', $start, $tong, $kmess) . '</div>';
}
break;
case 'edit':
if ($rights>=9) {
...
$id=$_GET[id];
$item=mysql_fetch_array(mysql_query("SELECT * FROM `hawaii` WHERE `id`='".$id."'"));
$shop=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$item[idshop]."'"));
  $xyz=mysql_num_rows(mysql_query("SELECT * FROM `hawaii` WHERE `id`='".$id."'"));
...
WHERE `id`='".$shop['id']."'
");
@mysql_query("UPDATE `hawaii` SET
`mattrang`='".$_POST[mattrang]."'
...
echo 'Thời hạn(nhập ngày): <input type="number" name="timesudung" value="'.$shop[timesudung].'"><br/>';

echo 'Nhập đá mặt trăng: <input type="number" name="mattrang" value="'.$item[mattrang].'"><br/>';
...
$checkit = mysql_result(mysql_query("select count(*) from `hawaii` where `id` = '".$id."'"),0);
if ($checkit == 0) {
	echo '<div class="omenu">Không có Item này !</div>';
} else {
mysql_query("DELETE FROM `hawaii` WHERE `id`='".$id."'");
    header ('Location : /hawaii/shop.php ');
}
}
break;
case 'add':
...
mysql_query("INSERT INTO `hawaii` SET
`idshop`='".$vatpham."',
`mattrang`='".$xu."'");
$tenvatpham=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$vatpham."'"));
$bot='[b]'.$login.' vừa thêm [red]'.$tenvatpham[tenvatpham].' [/red] vào shop hawaii![/b]';
...
echo '</select><br/>';
echo 'Cần mặt trăng: <input type="text" name="xu" size="5"><br/>';
...
$id = (int)$_GET['id'];
$check=mysql_num_rows(mysql_query("SELECT * FROM `hawaii` WHERE `id`='".$id."'"));
if ($check<1) {
...
$item=mysql_fetch_array(mysql_query("SELECT * FROM `hawaii` WHERE `id`='".$id."'"));
$shop=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$item[idshop]."'"));
echo'<div class="omenu"><center> <img src="/images/shop/'.$shop['id'].'.png"><br>Bạn có muốn mua <font color="ff007f">'.$shop['tenvatpham'].'  ('.($shop['timesudung'] ? thoigiantinh(floor($shop['timesudung'])).'' : 'Vĩnh viễn').')</font> với <b>'.number_format($item['mattrang']).' đá mặt trăng không </b>? <br>
...
echo'<div class="omenu">Bạn đã mua thành công: <b><font color="red"> '.$shop['tenvatpham'].'  ('.($shop['timesudung'] ? thoigiantinh(floor($shop['timesudung'])).'' : 'Vĩnh viễn').')</font></b></div>';
}else{
