...
switch($act) {
default:
$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `hawaii_db` "),0);
$req=mysql_query("SELECT * FROM `hawaii_db` ORDER BY `id` DESC LIMIT $start,$kmess");
...
if ($tong > $kmess){
echo '<div class="topmenu">' . functions::display_pagination('dacbiet.php?', $start, $tong, $kmess) . '</div>';
}
break;
case 'edit':
if ($rights>=9) {
...
$id=$_GET[id];
$item=mysql_fetch_array(mysql_query("SELECT * FROM `hawaii_db` WHERE `id`='".$id."'"));
$shop=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$item[idshop]."'"));
  $xyz=mysql_num_rows(mysql_query("SELECT * FROM `hawaii_db` WHERE `id`='".$id."'"));
...
@mysql_query("UPDATE `hawaii_db` SET
`luong`='".$_POST[luong]."',
...
mysql_query("DELETE FROM `hawaii_db` WHERE `id`='".$id."'");
    header ('Location : dacbiet.php ');

}
$checkit = mysql_result(mysql_query("select count(*) from `hawaii_db` where `id` = '".$id."'"),0);
if ($checkit == 0) {
...
*/
if (empty($vatpham) or empty($luong)  or empty($hsd)) {
echo '<div class="news">Không được bỏ trống chỗ nào hết!</div>';
} else {
mysql_query("INSERT INTO `hawaii_db` SET
`idshop`='".$vatpham."',
`luong`='".$luong."',
`timebd`='".$hsd."'");
$tenvatpham=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$vatpham."'"));
$bot='[b]'.$login.' vừa thêm [red]'.$tenvatpham[tenvatpham].' [/red] vào shop hawaii đặc biệt![/b]';
...
$id = (int)$_GET['id'];
$check=mysql_num_rows(mysql_query("SELECT * FROM `hawaii_db` WHERE `id`='".$id."'"));
if ($check<1) {
header('Location: dacbiet.php');
exit;
}
