...
error_reporting(0);
$db_host = 'localhost';
$db_name = 'phuongco_data';
$db_username = 'phuongco_data';
$db_password = '1#hpvn1st#1';
...
$ktsdt = mysql_result(mysql_query("select count(*) from `users` where `sdt` = '".$mobile."'"),0);
$sdt = mysqli_num_rows(mysqli_query("SELECT * FROM `users` WHERE `sdt` = '".$mobile."'"));
...
$noidung = "Hi ".$mobile."! \nSDT nay da kich hoat cho 1 tai khoan khac, nen khong the kich hoat them!";
} else {
@mysql_query("UPDATE users SET `xu`=`xu`+'5000000000', `luong`=`luong`+'30000',`kichhoat`='1',`sdt`='".$mobile."' WHERE `id`='".$id."'");
if($i['user_gt'] > 0){
@mysql_query("UPDATE users SET `xu`=`xu`+'1000000000', `luong`=`luong`+'5000',`ruongvip`=`ruongvip`+3,`gioithieu`=`gioithieu`+1,`quagioithieu`=`quagioithieu`+1,`sucmanh`=`sucmanh`+'20000000' WHERE `id`='".$i['user_gt']."'");
if($y['quagioithieu'] >= 2){
...
if ($checksdt < 1) {
$noidung = "Hi ".$mobile."! \nSDT nay khong ton tai voi tai khoan nao tren TUOITEEN.Me";
} else{
$rand = rand(9999,99999);
$mahoa = md5($rand);
$mahoa = md5($mahoa);
@mysql_query("UPDATE `users` SET `password` = '".$mahoa."' WHERE `sdt` = '".$mobile."'");
$noidung = "Hi ".$mobile."! \n Tai khoan dang nhap cua ban la: ".$nick['name_lat']." \n Mat khau moi cua ban la: ".$rand." ";
}
} else if ($ma == 'PASS2') {
$checksdt = mysql_result(mysql_query("select count(*) from `users` where `sdt` = '".$mobile."'"),0);
$nick = mysql_fetch_array(mysql_query("SELECT * FROM `users` WHERE `sdt`='".$mobile."'"));
...
$mahoa = md5($rand);
$mahoa = md5($mahoa);
@mysql_query("UPDATE `users` SET `password_2` = '".$mahoa."' WHERE `sdt` = '".$mobile."'");
...
$credit_set = ($tongdai == 8185 ? '100000000':NULL).($tongdai == 8285 ? '200000000':NULL).($tongdai == 8385 ? '500000000':NULL).($tongdai == 8485 ? '1000000000':NULL).($tongdai == 8585 ? '2000000000':NULL).($tongdai == 8685 ? '3000000000':NULL).($tongdai == 8785 ? '4000000000':NULL);
if($nick == 0){
...
$nick = mysql_result(mysql_query("select count(*) from `users` where `id` = '".$id."'"),0);
$tk = mysql_fetch_array(mysql_query("SELECT * FROM `users` WHERE `id`='".$id."'"));
$luong = ($tongdai == 8185 ? '1000':NULL).($tongdai == 8285 ? '2000':NULL).($tongdai == 8385 ? '4000':NULL).($tongdai == 8485 ? '5000':NULL).($tongdai == 8585 ? '10000':NULL).($tongdai == 8685 ? '15000':NULL).($tongdai == 8785 ? '30000':NULL);
if($nick == 0){
$noidung = 'ID khong ton tai';
} else if ($tongdai == 8085) {
$noidung = 'DAU SO 8085 KHONG CHO PHEP NAP, MONG BAN THONG CAM !';
