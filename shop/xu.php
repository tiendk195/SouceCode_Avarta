...
echo'<div class="phdr">Quay số > Quay số xu</div>';
switch($act) {
default:
...
$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `quayso`WHERE `xu`!='0'  "),0);
$vatpham = mysql_query("SELECT * FROM `quayso` WHERE `id`!=0 AND `xu`!='0' ORDER BY `id` ");
...
Echo'<div class="omenu">Bạn đã xóa thành công vật phẩm <font color="red">'.$shop['tenvatpham'].'</font> !!<a href="quaysoxu.php">Trở về!!</a></div>';
echo'</center>';
