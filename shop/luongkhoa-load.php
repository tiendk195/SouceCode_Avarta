...
$check=mysql_num_rows(mysql_query("SELECT * FROM `quayso` WHERE `id`='".$id."' AND `vndkhoa`>'0'"));
...
<br><font size="1"><i>'.$item[vndkhoa].' lượng khóa</br>
Tỉ lệ: ';
if ($item[tile]==1){
	echo' Không xác định';
} else {
...
$tile=floor($tile);
$rand=rand(1,$tile+2);
if ($datauser[luongkhoa]<$item[vndkhoa]) {
                       		    echo'<script>alert("Bạn không đủ lượng khóa để quay vật phẩm này!");</script>';
                       		    exit;
}
if ($x5quayso['soluong']>0) {
	mysql_query("UPDATE `vatpham` SET `soluong`=`soluong`-'1' WHERE `user_id`='".$user_id."' AND `id_shop`='126'");
}
mysql_query("UPDATE `users` SET `luongkhoa`=`luongkhoa`-'".$item[vndkhoa]."' WHERE `id`='".$user_id."'");
if ($rand==1) {
...
`id_shop`='".$shop[id]."'
");
$checknv2=mysql_num_rows(mysql_query("SELECT * FROM `nhiemvu_user` WHERE `user_id`='".$user_id."' AND `id_nv`='13'"));
if ($checknv2>0) {
mysql_query("UPDATE `nhiemvu_user` SET `tiendo`=`tiendo`+'1' WHERE `user_id`='".$user_id."' AND `id_nv`='13'");
}
