...
$check=mysql_num_rows(mysql_query("SELECT * FROM `quayso` WHERE `id`='".$id."' AND `xu`>'0'"));
...
<br><font size="1"><i>'.$item[xu].' xu</br>
Tỉ lệ: ';
if ($item[tile]==1){
	echo' Không xác định';
} else {
...
if ($datauser[xu]<$item[xu]) {
                       		    echo'<script>alert("Bạn không đủ xu để quay vật phẩm này!");</script>';
                       		    exit;
}
if ($x5quayso['soluong']>0) {
