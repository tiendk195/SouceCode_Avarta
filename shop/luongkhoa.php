...
echo'<div class="phdr">Quay số > Quay số lượng khóa</div>';
switch($act) {
default:
...
$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `quayso`WHERE `vndkhoa`!='0'  "),0);
$vatpham = mysql_query("SELECT * FROM `quayso` WHERE `id`!=0 AND `vndkhoa`!='0' ORDER BY `id` ");
...
function thongtin_vp(id)
{
    $.ajax(
        {
            url : 'luongkhoa-load.php?thongtin_vp',
            data : {id : id},
            type : 'POST',
            success : function(d)
            {
