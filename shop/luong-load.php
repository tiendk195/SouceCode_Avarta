...
$check=mysql_num_rows(mysql_query("SELECT * FROM `quayso` WHERE `id`='".$id."' AND `vnd`>'0'"));
...
<br><font size="1"><i>'.$item[vnd].' lượng</br>
Tỉ lệ: ';
if ($item[tile]==1){
	echo' Không xác định';
} else {
...
$tile=floor($tile);
$rand=rand(1,$tile+2);
if ($datauser[luong]<$item[vnd]) {
                       		    echo'<script>alert("Bạn không đủ lượng để quay vật phẩm này!");</script>';
                       		    exit;
}
if ($x5quayso['soluong']>0) {
