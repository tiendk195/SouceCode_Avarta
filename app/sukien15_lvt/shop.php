...
Echo'<div class="box_forums"><br/><div class="homeforum"><div class="icon-home"><div class="home">Khu mua sắm</div></div></div></div><div class="phdr">Đổi quà 1.5</div>';
echo'<div class="omenu"><font color="red"><b>Bạn đang có '.number_format($datauser['diemsk15']).' điểm sự kiện</b></font></div>';
?>
<!--Edit by Lethinh-->
<?php
...
}Else IF($datauser['diemsk15'] < 0){
Echo'<div class="omenu"><center><b><font color="red">Bạn cần ít nhất 1 điểm sự kiện để vào đây</font></b></center></div>';
}Else{
switch($act){
case'mua':
$vp=intval($_GET['id']);
$p=mysql_fetch_array(mysql_query("SELECT * FROM `shop15` WHERE `id`='".$vp."'"));
$r=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$p['vatpham']."'"));
IF($r['timesudung']){
$r['timesudung'] = $r['timesudung'] + time();
}
$n=mysql_num_rows(mysql_query("SELECT * FROM `shop15` WHERE `id`='".$vp."'"));
IF($n<1){
Header('Location: /');
Exit;
} 
...
IF($datauser['diemsk15'] >= $p['gia']){
mysql_query("UPDATE `users` SET `diemsk15`=`diemsk15`-'".$p['gia']."' WHERE `id`='".$user_id."'");
...
Echo'<div class="omenu"><center><b><font color="red"><img src="/images/shop/'.$r['id'].'.png"><br>Bạn có chắc chắn đổi '.$r['tenvatpham'].' với '.number_format($p['gia']).' Điểm sự kiện không??</font></b><form method="post"><input type="submit" name="mua" value="Đổi"></center></div>';
break;
default:
$e=mysql_query("SELECT * FROM
`shop15` WHERE `id`!=0 ORDER BY `id` DESC LIMIT $start, $kmess");
while($s=mysql_fetch_array($e)){
$res=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$s['vatpham']."'"));
$res2=mysql_fetch_array(mysql_query("SELECT * FROM `shop15` WHERE `id`='".$s['id']."'"));

Echo'<div class="omenu" style="padding: 0px"><table width="100%" border="0" cellspacing="0" class="profile-info"><tbody><tr><td class="left-info" width="25%">
<img src="/images/shop/'.$res['id'].'.png"></td><td class="right-info" width="75%">
Tên: <b>'.$res['tenvatpham'].'</b> <br>
Cần: <b>'.number_format($res2['gia']).' Điểm sự kiện</b>
<br>Hạn sử dụng: '.($r['timesudung'] ? thoigiantinh(floor($r['timesudung'])).'' : '
<b><font color="red">Vĩnh viễn</font></b>').'</div><div class="menu"><a href="?act=mua&id='.$s['id'].'">
<button> Đổi </button></a>';
if ($datauser['rights'] >=9) {
...
$total=mysql_result(mysql_query("SELECT COUNT(*) FROM `shop15` WHERE `id`!=0"), 0);
IF($total > $kmess){
...
IF($datauser['rights'] >= 9){
mysql_query("UPDATE `shop15` SET `gia`= '".$gia."' WHERE `id`='".$vp."'");
...
mysql_query("INSERT `shop15` SET `gia`= '".$gia."',`vatpham`= '".$idshop."' ");
...
IF($datauser['rights'] >= 9){
	mysql_query("DELETE FROM `shop15` WHERE `id`='".$vp."'");
