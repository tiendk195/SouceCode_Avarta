...
Echo'<div class="phdr">Đổi quà 1.6</div>';
echo'<div class="omenu"><font color="red"><b>Bạn đang có '.number_format($datauser['diemsk16']).' điểm sự kiện</b></font></div>';
?>
<!--Edit by Lethinh-->
<?php
...
}Else IF($datauser['diemsk16'] < 100 ){
Echo'<div class="omenu"><center><b><font color="red">Bạn cần ít nhất 100 điểm sự kiện để vào đây</font></b></center></div>';
}Else{
switch($act){
case'mua':
$vp=intval($_GET['id']);
...
IF($datauser['diemsk16'] >= $p['gia']){
mysql_query("UPDATE `users` SET `diemsk16`=`diemsk16`-'".$p['gia']."' WHERE `id`='".$user_id."'");
...
echo'<div class="lt" style="text-align:center;"><select name="doiqua" input type="submit" onchange="location = this.options[this.selectedIndex].value;">
    Đổi quà: <br>
<option value="shop.php"/>Đổi quà</option>
...
<font color="ff007f">Vật phẩm:</font>
         '.$res['tenvatpham'].' '.($r['timesudung'] ? thoigiantinh(floor($r['timesudung'])).'' : ' <font color="red">(Vĩnh viễn)</font>').' <br>
          <font color="ff007f">Cần:</font> '.number_format($res2['gia']).'  điểm sự kiện</font><br>
