...
switch($act){
	default:
echo'<div class="phdr">Game > Sự kiện Pokemon</div>';
$p=mysql_fetch_array(mysql_query("SELECT * FROM `khupokemon` WHERE `user_id`= '".$user_id."'"));
if($gio > 1900 && $gio < 2000 ){

echo'<center><div class="omenu"><div class="lt">Khung giờ vàng (19h đến 20h)</br><font color="red">Khu của bạn: '.$p['khu'].'</font></div></div></center>';
}

$e=mysql_query("SELECT * FROM `khuboss15` ORDER BY `id`");
...
case 'khu':
    $vp=intval($_GET['id']);
$p=mysql_fetch_array(mysql_query("SELECT * FROM `khupokemon` WHERE `user_id`= '".$user_id."'"));
$n=mysql_num_rows(mysql_query("SELECT * FROM `khupokemon` WHERE `khu`= '".$vp."' AND `user_id`= '".$user_id."'"));
IF($vp>30 or $vp<1){
Header('Location: /');
Exit;
} 
...
echo'<div class="phdr">Game > Sự kiện Pokemon > Khu '.$vp.'</div>';
...
echo'<center>
<font color="white" style="text-shadow: 0 0 0.2em #F08080, 0 0 0.2em #F08080,  0 0 0.2em #F08080"> Pokemon hệ:</font>
<font color="white" style="text-shadow: 0 0 0.2em #FF0000, 0 0 0.2em #FF0000,  0 0 0.2em #FF0000"> <b>'.$he.'</b> <img src="/images/vatpham/'.$img.'.png"></font><br>
<img src="images/'.$p['loaiboss'].'.gif" alt="icon"><br>
<a href="bat.php?id='.$vp.'"> <input id="submit" type="submit" name="submit" value="Bắt"></a></br></br>

</center>
';
}

echo'</center> </div><div class="buico"></div><div class="nen" style="height:85px; margin:0;"><a href="map.php"><img src="/app/images/k'.$vp.'.png"></a>';
//-- Online Topic ---//
