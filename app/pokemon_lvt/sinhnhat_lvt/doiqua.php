...
case'mua':
$vp=intval($_GET['id']);
$p=mysql_fetch_array(mysql_query("SELECT * FROM `shopsinhnhat` WHERE `id`='".$vp."'"));
$r=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$p['vatpham']."'"));
IF($r['timesudung']){
$r['timesudung'] = $r['timesudung'] + time();
}
$n=mysql_num_rows(mysql_query("SELECT * FROM `shopsinhnhat` WHERE `id`='".$vp."'"));
IF($n<1){
Header('Location: /');
Exit;
} 
...
`shopsinhnhat` WHERE `id`!=0 ORDER BY `id` DESC LIMIT $start, $kmess");
while($s=mysql_fetch_array($e)){
$res=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$s['vatpham']."'"));
$res2=mysql_fetch_array(mysql_query("SELECT * FROM `shopsinhnhat` WHERE `id`='".$s['id']."'"));
...
$total=mysql_result(mysql_query("SELECT COUNT(*) FROM `shopsinhnhat` WHERE `id`!=0"), 0);
IF($total > $kmess){
...
IF($datauser['rights'] >= 9){
mysql_query("UPDATE `shopsinhnhat` SET `gia`= '".$gia."' WHERE `id`='".$vp."'");
...
IF($datauser['rights'] >= 9){
	mysql_query("DELETE FROM `shopsinhnhat` WHERE `id`='".$vp."'");
