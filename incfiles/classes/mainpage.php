...
class mainpage {
    ...
    private function news() {
        ...
        if ($this->settings['view'] > 0) {
            ...
            $req = mysql_query("SELECT * FROM `news` WHERE `time` > '$reqtime' ORDER BY `time` DESC LIMIT 1");
            if (mysql_num_rows($req) > 0) {
                ...
                while (($res = mysql_fetch_array($req)) !== false) {
                    ...
                    $news .= '<div class="lucifer"><marquee direction="left" loop="infinite">';
                    ...
                    $news .= '</span> </marquee> </td> </tr> </tbody> </table></div>';
                    ++$i;
                }
                return $news;
            } else {
