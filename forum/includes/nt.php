...
$th = isset($_POST['th']) ? functions::check(mb_substr(htmlspecialchars($_POST['th']), 0, 100)) : '';
$msg = isset($_POST['msg']) ? functions::checkin(htmlspecialchars($_POST['msg'])) : '';
$tags = isset($_POST['tags']) ? htmlspecialchars($_POST['tags']) : '';   $msg = preg_replace_callback('~\\[url=(http://.+?)\\](.+?)\\[/url\\]|(http://(www.)?[0-9a-zA-Z\.-]+\.[0-9a-zA-Z]{2,6}[0-9a-zA-Z/\?\.\~&amp;_=/%-:#]*)~', 'forum_link', $msg);
...
$check = functions::checkout($th, 1, 0); 
$url = $check;
if(preg_match('|Bán|',$url)|| preg_match('|bán|',$url) || preg_match('|BÁN|',$url))  
$error[] = $lng_forum['error_topic_name_lenght'];
...
function get_link ($input) {
  preg_match_all ('#\[adubgfdg\](.+?)\[/dịghgf\]#i',$input,$regex);
  return $regex[1];
  }
  ...
  $contents = preg_replace ('#\[img\](.+?)\[/img\]#is','[img]$1[/img]',replace_link ($list,$new,$contents));
    ...
    if (!$error) {
        ...
        mysql_query("INSERT INTO `forum` SET
            ...
            `curators` = ''
        ") or exit(__LINE__ . ': ' . mysql_error());
								mysql_query("UPDATE `users` SET `tim1st` = `tim1st`+ '200' WHERE `id` = '".$user_id."'");
$textt = 'Bạn nhận được 200 tim <img src="https://i.imgur.com/ZL9ehAx.gif"> từ Đăng bài viết';
...
thời gian giữa 2 bài viết tối thiểu là 60 giây và giữa 2 comment là 20 giây<br/><a href="/forums/dang-bai-' . $id . '.html">Click để quay về diễn đàn !</a></center>';
 ...
 echo '<form name="form" action="/forums/dang-bai-' . $id . '.html" method="post">' .
        '<div class="menu">' .
        '<b>Tiêu Đề:</b><br/>';
        echo'	<input type="text" size="40" style="height: 25px;" maxlength="150" name="th" value="' . $th . '"/></p>' .
        '<b>Nội Dung:</b><br/>';
		
    echo '<textarea rows="3" name="msg">' . (isset($_POST['msg']) ? functions::checkout($_POST['msg']) : '') . '</textarea></p>' .
         '<font color="red"><b> Các Bạn Có Thể Up Ảnh Lên [<a href="/upanh"><b> Tại Đây </b></a> ]<br/>Sau Dẫn Link BBcode Vào Nơi Muốn Hiển Thị</font>'  ;
    if ($set_user['translit']) {
        echo '<br /><input type="checkbox" name="msgtrans" value="1" ' . (isset($_POST['msgtrans']) ? 'checked="checked" ' : '') . '/> ' . $lng['translit'];
    }
    $token = mt_rand(1000, 100000);
    $_SESSION['token'] = $token;
if($id == 2 AND $datauser['rights'] < 9){
echo'<div class="news">Không có quyền</div></div>';
} else
if ($datauser['kichhoat'] ==0) {
    
echo'<div class="news">Vui lòng kích hoạt để sử dụng chức năng này </div></div>';
} else {
