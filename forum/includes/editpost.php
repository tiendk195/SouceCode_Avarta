        ...
        default:
            ...
            */
            ...
            if (isset($_POST['submit'])) {
                ...
                echo '<textarea rows="5" name="msg">' . (empty($_POST['msg']) ? htmlentities($res['text'], ENT_QUOTES, 'UTF-8') : functions::checkout(htmlspecialchars($_POST['msg']))) . '</textarea><br/>';
                if ($set_user['translit'])
                    echo '<input type="checkbox" name="msgtrans" value="1" ' . (isset($_POST['msgtrans']) ? 'checked="checked" ' : '') . '/> ' . $lng['translit'];
                echo '</p><p><input type="submit" name="submit" value="Chỉnh sửa" style="width: 50px; cursor: pointer;"/> ' .
                     '</p></form></div></div>';
            }
    }
} else {
