...
Echo '<div class="box_forums"><br/><div class="homeforum"><div class="icon-home"><div class="home">Kết hôn</div></div></div></div><div class="phdr">Tin hôn sự</div>';
$k=mysql_query("SELECT * FROM `kethon` WHERE `nguoi_ay`='".$user_id."' AND `dongy`='0' ORDER BY `time` LIMIT 1");while($khs=mysql_fetch_assoc($k)){if($khs>0){$ths=1;}}if($ths<=0){echo'<div class="omenu">Hiện tại chưa có tin hôn sự nào !!</div>';}else{
...
if ($kethon['dongy']==0) {
$bot='[red]'.$datauser['name'] .'[/red] đã đồng ý lời kết hôn [red]'.$cc['name'].'[/red]. Chúc mừng nào !!';
