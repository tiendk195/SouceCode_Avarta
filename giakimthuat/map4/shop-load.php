...
$check=mysql_num_rows(mysql_query("SELECT * FROM `giakimthuat_shopjones` WHERE `id`='".$vatpham."' "));

if($check < 1){
                		    echo'<script>alert("Lỗi không có vật phẩm này!!");</script>';
}else{
        $info = mysql_fetch_array(mysql_query("SELECT * FROM `giakimthuat_shopjones` WHERE `id`='".$vatpham."'"));
...
$check=mysql_num_rows(mysql_query("SELECT * FROM `giakimthuat_shopjones_doiqua` WHERE `id`='".$vatpham."' "));

if($check < 1){
                		    echo'<script>alert("Lỗi không có vật phẩm này!!");</script>';
}else{
        $info = mysql_fetch_array(mysql_query("SELECT * FROM `giakimthuat_shopjones_doiqua` WHERE `id`='".$vatpham."'"));
