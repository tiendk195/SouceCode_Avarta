...
echo '<div class="list1"><center><a href="showimg.php?id='.$res['id'].'"><img style="padding:2px;border:1px solid #D2D1D1;" src="'.$res['url'].'" width="100" height="70" alt="Hình ảnh upload Gocpho.biz"></a></center><div class="list1" style="background:#EAEEFC;">'.functions::display_date($res['time']).' ('.$res['size'].'KB) » <a href="delete.php?id='.$res['id'].'">Xóa</a></div></div>';
}
} else {
echo '<div class="list1">'.nick($id).' chưa upload ảnh nào </div>';
}
if ( $tong > $kmess ){echo '<div class="quote">' . functions :: display_pagination ( 'file.php?page=' , $start , $tong , $kmess ) . '</div>' ;}
