...
echo '<div class="list1"><center><a href="showimg.php?id='.$res['id'].'"><img style="padding:2px;border:1px solid #D2D1D1;" src="'.$res['url'].'" style="max-width: 98%;" alt="Hình ảnh upload"></a></center><div class="list1" style="background:#EAEEFC;">'.nick($res['user']).' » '.functions::display_date($res['time']).' » '.$res['size'].'KB</div></div>';
}
if ( $tong > $kmess ){echo '<div class="phantrang">' . functions::display_pagination('index.php?', $start, $tong, $kmess) . '</div>' ;}
