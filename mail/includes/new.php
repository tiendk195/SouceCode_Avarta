...
$total = mysql_result(mysql_query("SELECT COUNT(*) FROM (SELECT DISTINCT `user_id` FROM `cms_mail` WHERE `from_id`='$user_id' AND `read`<='1' AND `spam`='0') a;"), 0);
if($total == 1) {
    //Если все новые сообщения от одного итого же чела показываем сразу переписку
    $max = mysql_result(mysql_query("SELECT `user_id`, count(*) FROM `cms_mail` WHERE `from_id`='$user_id' AND `read`<='1' AND `spam`='0' GROUP BY `user_id`"), 0);
    Header('Location: /mail/mail.php?id='.$max);
    echo '</div>';
    exit();
}
if($total) {
	...
	$query = mysql_query("SELECT `users`.* FROM `cms_mail`
		...
		WHERE `cms_mail`.`from_id`='" . $user_id . "' AND `cms_mail`.`read`<='1' AND `cms_mail`.`spam`='0' GROUP BY `cms_mail`.`user_id` ORDER BY `cms_contact`.`time` DESC LIMIT " . $start . "," . $kmess
    );
	for ($i = 0; ($row = mysql_fetch_assoc($query)) !== false; ++$i) {
		echo $i % 2 ? '<div class="omenu">' : '<div class="omenu">';
		$subtext = '<a href="/mail/mail.php?id=' . $row['id'] . '">' . $lng_mail['correspondence'] . '</a> | <a href="index.php?act=deluser&amp;id=' . $row['id'] . '">' . $lng['delete'] . '</a> | <a href="index.php?act=ignor&amp;id=' . $row['id'] . '&amp;add">' . $lng_mail['ban_contact'] . '</a>';
		...
		$new_count_message = mysql_result(mysql_query("SELECT COUNT(*) FROM `cms_mail` WHERE `cms_mail`.`user_id`='{$row['id']}' AND `cms_mail`.`from_id`='$user_id' AND `read`<='1' AND `delete`!='$user_id' AND `spam`='0';"), 0);
		$arg = array(
...
echo '<div class="omenu"><a href="/mail/thu-di.html"><img src="/mail/images/tinnhan.png"/> <font color="003366">Hộp thư đi</font></a></div></div>';
