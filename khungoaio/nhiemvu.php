...
<div class="menu"><a href="?act=danhsachnhiemvuhangngay"><img src="/images/vao.png"> Danh sách Nhiệm vụ Hằng ngày</a></div>
...
$all=mysql_fetch_array(mysql_query("SELECT max(id) FROM `nhiemvu`"));  
$rand=rand(1,$all['max(id)']);
$rand1=rand(1,$all['max(id)']);

$rand2=rand(1,$all['max(id)']);
...
mysql_query("UPDATE `timenhiemvuhn` SET `time`='".time()."'WHERE user_id = '".$user_id."'");
mysql_query("INSERT INTO `nhiemvu_user` SET
`user_id`='".$user_id."',
`loai`='hangngay',
`id_nv`='".$rand."'");
...
exit;
	...
	echo'<div class="omenu">Bạn đã nhận nhiệm vụ hằng ngày rồi! <a href="?act=myhangngay">Bấm vào đây để xem</a></div>';

break;
case 'nhanthuonghangngay':
echo '<div class="phdr">Nhận thưởng nhiệm vụ</div>';

$id=(int)$_GET['id'];
$check=mysql_fetch_array(mysql_query("SELECT * FROM `nhiemvu_user` WHERE `id_nv`='".$id."' AND `loai`='hangngay' AND`user_id`='".$user_id."'"));
if ($check<1) {
echo '<div class="omenu">Không có nhiệm vụ này!</div></div>';
require('../incfiles/end.php');
exit;
}
$res=mysql_fetch_array(mysql_query("SELECT * FROM `nhiemvu_user` WHERE `id_nv`='".$id."' AND `loai`='hangngay' AND`user_id`='".$user_id."'"));
$nv=mysql_fetch_array(mysql_query("SELECT * FROM `nhiemvu` WHERE `id`='".$id."'"));
$nhan=mysql_num_rows(mysql_query("SELECT * FROM `nhiemvu_user` WHERE `id_nv`='".$id."' AND `loai`='hangngay' AND `user_id`='".$user_id."'"));
if ($res['nhanthuong']==1) {
...
mysql_query("UPDATE `users` SET `diemnangdong`= `diemnangdong` + '10' WHERE `id`='".$user_id."'");

echo '<div class="omenu">Nhận thưởng thành công nhiệm vụ <b>'.$nv['tennhiemvu'].'</b></br>
Bạn nhận được 10 điểm năng động <a href="nhiemvu.php"><input type="button" value="Quay lại"></a></div>';
}
break;
case 'nhanthuongtanthu':
echo '<div class="phdr">Nhận thưởng nhiệm vụ</div>';

$id=(int)$_GET['id'];
$check=mysql_fetch_array(mysql_query("SELECT * FROM `nhiemvu_user` WHERE `id_nv`='".$id."' AND `loai`='tanthu' AND`user_id`='".$user_id."'"));
if ($check<1) {
echo '<div class="omenu">Không có nhiệm vụ này!</div></div>';
require('../incfiles/end.php');
exit;
}
$res=mysql_fetch_array(mysql_query("SELECT * FROM `nhiemvu_user` WHERE `id_nv`='".$id."' AND `loai`='tanthu' AND`user_id`='".$user_id."'"));
$nv=mysql_fetch_array(mysql_query("SELECT * FROM `nhiemvu` WHERE `id`='".$id."'"));
$nhan=mysql_num_rows(mysql_query("SELECT * FROM `nhiemvu_user` WHERE `id_nv`='".$id."' AND `loai`='tanthu' AND `user_id`='".$user_id."'"));
if ($res['nhanthuong']==1) {
echo '<div class="omenu">Bạn đã nhận thưởng nhiệm vụ <b>'.$nv['tennhiemvu'].'</b> rồi nhé <a href="nhiemvu.php"><input type="button" value="Quay lại"></a></div>';
} else if ($nhan<1) {
echo '<div class="omenu">Bạn chưa nhận nhiệm vụ này nhé <a href="nhiemvu.php"><input type="button" value="Quay lại"></a></div>';
...
$total = mysql_result(mysql_query("SELECT COUNT(*) FROM `nhiemvu_user` WHERE  `loai`='tanthu' AND `nhanthuong`!='1' AND `user_id`='".$user_id."' "),0);

if ($total<1){
	echo'<div class="omenu"><center><font color="red">Nhiệm vụ tân thủ hiện tại của bạn đang trống</font></center></div></div>';
	require('../incfiles/end.php');
exit;

} 
echo'<div class="omenu"><center><font color="red">Nhiệm vụ tân thủ hiện tại của bạn là:</font></center></div>';
$req=mysql_query("SELECT * FROM `nhiemvu_user` WHERE `user_id`='".$user_id."' AND `id_nv`!='0' AND `nhanthuong`!='1' AND `loai`='tanthu' ORDER BY `id_nv` DESC LIMIT $start, $kmess");
$i=1;
while ($res=mysql_fetch_array($req)) {
...
<b><font color="#003366">Tên nhiệm vụ: </font></b>'.$nv['tennhiemvu'].'<br>
...
echo'<a href="?act=nhanthuongtanthu&id='.$res['id_nv'].'"><input type="submit" value="Nhận thưởng"></a><br/>';
} else {
...
if($total > $kmess) {
echo '<div class="phantrang">'.functions::display_pagination('?act=mytanthu&',$start,$total,$kmess).'</div>';
}
break;
case 'myhangngay':
echo '<div class="phdr">Nhiệm vụ của tôi</div>';
...
$total = mysql_result(mysql_query("SELECT COUNT(*) FROM `nhiemvu_user` WHERE  `loai`='hangngay' AND `nhanthuong`!='1' AND `user_id`='".$user_id."' "),0);

if ($total<1){
	echo'<div class="omenu"><center><font color="red">Nhiệm vụ hằng ngày hiện tại của bạn đang trống</font></center></div></div>';
	require('../incfiles/end.php');
exit;
...
$req=mysql_query("SELECT * FROM `nhiemvu_user` WHERE `user_id`='".$user_id."' AND `id_nv`!='0' AND `nhanthuong`!='1' AND `loai`='hangngay'  ORDER BY `id_nv` DESC LIMIT $start, $kmess");
$i=1;
while ($res=mysql_fetch_array($req)) {
$nv=mysql_fetch_array(mysql_query("SELECT * FROM `nhiemvu` WHERE `id`='".$res['id_nv']."'"));
...
<b><font color="blue"> Tiến độ: ( '.$res['tiendo'].' / '.$nv['hoanthanh'].' )</font><center>';
if ($res['tiendo']>=$nv['hoanthanh']){
echo'<a href="?act=nhanthuonghangngay&id='.$res['id_nv'].'"><input type="submit" value="Nhận thưởng"></a><br/>';
} else {
echo'<input  type="submit" value="Chưa hoàn thành">';
}
echo'</center></b></div>';
...
if($total > $kmess) {
echo '<div class="phantrang">'.functions::display_pagination('?act=myhangngay&',$start,$total,$kmess).'</div>';
}

break;
case 'danhsachnhiemvuhangngay':
echo '<div class="phdr">Danh sách nhiệm vụ hằng ngày</div>';
$req=mysql_query("SELECT * FROM `nhiemvu` ORDER BY `id` DESC LIMIT $start, $kmess ");
$i=1;
while ($res=mysql_fetch_array($req)) {
echo'<div class="omenu"><b><font color="#003366">Nhiệm vụ hằng ngày '.$res['id'].':</font> </b>
<br><b><font color="ff3399">Phần thưởng :</font></b>  '.$res['phanthuong'].'<center>';

echo'
<a href="?act=chitiethangngay&amp;id='.$res['id'].'"><input type="submit" value="Chi tiết"></a>';

echo'</div>';
++$i;
}
$total = mysql_result(mysql_query("SELECT COUNT(*) FROM `nhiemvu` "),0);

if($total > $kmess) {
echo '<div class="phantrang">'.functions::display_pagination('?act=danhsachnhiemvuhangngay&',$start,$total,$kmess).'</div>';
}
break;
}
echo '</div>';
