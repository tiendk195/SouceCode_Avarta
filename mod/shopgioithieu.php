...
require('../incfiles/head.php');
echo'<div class="phdr">Đổi Điểm Giới Thiệu</div>';
echo'<div class="omenu"><center>Bạn đang có <b> <font color="red">'.number_format($datauser['diemgioithieu']).'</font></b> điểm giới thiệu </center></div>';
...
switch($act) {
default:
$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `shopgioithieu` "),0);
$req=mysql_query("SELECT * FROM `shopgioithieu` ORDER BY `id` DESC LIMIT $start,$kmess");
...
echo'</font></b><br>
Cần: <font color="red">'.number_format($res['diem']).'</font> Điểm giới thiệu<br>
...
if ($tong > $kmess){
echo '<div class="topmenu">' . functions::display_pagination('shopgioithieu.php?', $start, $tong, $kmess) . '</div>';
}
break;
case 'edit':
if ($rights>=9) {
...
$id=$_GET[id];
$item=mysql_fetch_array(mysql_query("SELECT * FROM `shopgioithieu` WHERE `id`='".$id."'"));
$shop=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$item[idshop]."'"));
  $xyz=mysql_num_rows(mysql_query("SELECT * FROM `shopgioithieu` WHERE `id`='".$id."'"));
...
@mysql_query("UPDATE `shopgioithieu` SET
`diem`='".$_POST[diemgioithieu]."',
...
echo 'Nhập điểm: <input type="number" name="diemgioithieu" value="'.$item[diem].'"><br/>';
...
mysql_query("DELETE FROM `shopgioithieu` WHERE `id`='".$id."'");
    header ('Location : shopgioithieu.php ');

}
$checkit = mysql_result(mysql_query("select count(*) from `shopgioithieu` where `id` = '".$id."'"),0);
if ($checkit == 0) {
...
mysql_query("INSERT INTO `shopgioithieu` SET
`idshop`='".$vatpham."',
`diem`='".$diem."',
`timebd`='".$hsd."'");
$tenvatpham=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$vatpham."'"));
$bot='[b]'.$login.' vừa thêm [red]'.$tenvatpham[tenvatpham].' [/red] vào shop giới thiệu![/b]';
...
$id = (int)$_GET['id'];
$check=mysql_num_rows(mysql_query("SELECT * FROM `shopgioithieu` WHERE `id`='".$id."'"));
if ($check<1) {
header('Location: shopgioithieu.php');
exit;
}
...
echo'</font> với <b>'.number_format($item['diem']).' Điểm giới thiệu không </b>? <br>
...
if($datauser['diemgioithieu'] >= $item['diem']){
    
    if ($item['timebd'] !=0)
{
$timesd= $item['timebd']*24*3600+time();
}
mysql_query("UPDATE `users` SET `diemgioithieu` = `diemgioithieu` - '".$item['diem']."' WHERE `id` = '".$user_id."'");
