...
$check = mysql_num_rows(mysql_query("SELECT * FROM `ssm_quavip` WHERE `id`='".$vatpham."'"));
if($check < 1){
                		    echo'<script>alert("Vật phẩm không tồn tại!!");</script>';
                		    exit;
}
...
if ($tt['kichhoat']==0 ){
                   		    ...
                   		    echo'<script>alert("Vật phẩm chỉ dành cho SSM VIP!!");</script>';
                		    exit;
}
if ($tt['level']<$vatpham ){
...
$tt2=mysql_fetch_array(mysql_query("SELECT * FROM `ssm_vip_user` WHERE `user_id`='".$user_id."' AND `level` = '".$vatpham."' "));
if ($tt2['nhanthuong']==1 ){
...
$p=mysql_fetch_array(mysql_query("SELECT * FROM `ssm_quavip` WHERE `id`='".$vatpham."'"));
if ($p['loai']=='vatpham'){
$ssm=mysql_fetch_array(mysql_query("SELECT * FROM `shopvatpham` WHERE `id`='".$p['id_loai']."'"));

	 	 mysql_query("UPDATE `ssm_vip_user` SET `nhanthuong`= '1' WHERE `user_id`='{$user_id}' AND `level`='{$p['id']}' ");
