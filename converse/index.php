...
`name` = '$from',
`text` = '" . tagnick(mysql_real_escape_string($msg)) . "',
`ip` = '" . core::$ip . "',
...
mysql_query("UPDATE `users` SET `postguest` = '$postguest', `lastpost` = '" . time() - 5 . "' WHERE `id` = '$user_id'");
}
...
echo '<div class="phdr">Phòng chat</div>';
if(empty($datauser['kichhoat'])){
echo ' Bạn chưa kích hoạt tài <a href="/users/checkpoint.php">Nhấn Vào Đây</a> để kích hoạt</b>></font>';
}
else {
if ($total) {
if (isset($_SESSION['ga']) && $rights >= "1") {
...
$res_g = mysql_fetch_assoc($req_g);$res['lastdate'] = $res_g['lastdate'];
}
