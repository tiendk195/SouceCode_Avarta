...
require('../incfiles/head.php');
echo'<div class="phdr">Shop điểm tích lũy &gt; Đổi quà &gt; </div>';
echo'<div class="omenu"><center>Bạn đang có <b> <font color="red">'.number_format($datauser['diemtichluynt']).'</font></b> điểm tích lũy </center></div>';
...
switch($act) {
default:
$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `shoptichluynt` "),0);
$req=mysql_query("SELECT * FROM `shoptichluynt` ORDER BY `id` DESC LIMIT $start,$kmess");
...
echo'</font></b><br>
Cần: <font color="red">'.number_format($res['diem']).'</font> Điểm tích lũy<br>
...
if ($tong > $kmess){
echo '<div class="topmenu">' . functions::display_pagination('doidiem.php?', $start, $tong, $kmess) . '</div>';
}
break;
case 'edit':
if ($rights>=9) {
...
$id=$_GET[id];
$item=mysql_fetch_array(mysql_query("SELECT * FROM `shoptichluynt` WHERE `id`='".$id."'"));
$shop=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$item[idshop]."'"));
  $xyz=mysql_num_rows(mysql_query("SELECT * FROM `shoptichluynt` WHERE `id`='".$id."'"));
...
@mysql_query("UPDATE `shoptichluynt` SET
`diem`='".$_POST[shoptichluynt]."',
...
echo 'Nhập điểm: <input type="number" name="shoptichluynt" value="'.$item[diem].'"><br/>';
...
mysql_query("DELETE FROM `shoptichluynt` WHERE `id`='".$id."'");
    header ('Location : doidiem.php ');

}
$checkit = mysql_result(mysql_query("select count(*) from `shoptichluynt` where `id` = '".$id."'"),0);
if ($checkit == 0) {
...
mysql_query("INSERT INTO `shoptichluynt` SET
`idshop`='".$vatpham."',
`diem`='".$diem."',
`timebd`='".$hsd."'");
$tenvatpham=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$vatpham."'"));
$bot='[b]'.$login.' vừa thêm [red]'.$tenvatpham[tenvatpham].' [/red] vào shop tích lũy[b]';
...
$id = (int)$_GET['id'];
$check=mysql_num_rows(mysql_query("SELECT * FROM `shoptichluynt` WHERE `id`='".$id."'"));
if ($check<1) {
header('Location: doidiem.php');
exit;
}
...
echo'</font> với <b>'.number_format($item['diem']).' Điểm tích lũy không </b>? <br>
...
if($datauser['diemtichluynt'] >= $item['diem']){
    
    if ($item['timebd'] !=0)
{
$timesd= $item['timebd']*24*3600+time();
}
mysql_query("UPDATE `users` SET `diemtichluynt` = `diemtichluynt` - '".$item['diem']."' WHERE `id` = '".$user_id."'");
