...
case'khu':
$vp=intval($_GET['id']);
$p=mysql_fetch_array(mysql_query("SELECT * FROM `ngocrong_nuituyet` WHERE `id`='".$vp."'"));

$n=mysql_num_rows(mysql_query("SELECT * FROM `ngocrong_nuituyet` WHERE `id`='".$vp."'"));
IF($n<1){
Header('Location: /');
Exit;
} 
...
//////
mysql_query("UPDATE `users` SET `vitri` = '554' +'".$vp."' WHERE `id` = '".$user_id."'");

$online_u = mysql_result(mysql_query("SELECT COUNT(*) FROM `users` WHERE `lastdate` > " . (time() - 300) . " AND `vitri` = '554'+'".$vp."'"), 0);
$totalonline = $online_u;
$q = @mysql_query("select * from `users` where `lastdate` > " . (time() - 300) . " AND `vitri` = '554'+'".$vp."';");
...
default:
echo'<div class="phdr">Map Núi tuyết </div>';

    $e=mysql_query("SELECT * FROM `ngocrong_nuituyet` ORDER BY `id`");
