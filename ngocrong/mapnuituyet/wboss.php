...
$wboss = mysql_fetch_array(mysql_query("SELECT * FROM `ngocrong_nuituyet` WHERE `id` = '$id'"));
$n=mysql_num_rows(mysql_query("SELECT * FROM `ngocrong_nuituyet` WHERE `id`='".$id."'"));
IF($n<1){
Header('Location: /');
Exit;
}
...
echo'<script>alert("'.$wboss['tenboss'].' sẽ xuất hiện trong '.$timeboss.'s nữa!");</script>';
   ...
   mysql_query("UPDATE `ngocrong_nuituyet` SET `hp`=`hpfull` WHERE `id`='$id'");
...
mysql_query("UPDATE `ngocrong_nuituyet` SET `hp`='0' , `time` = {$timeboss} WHERE `id`='$id'");
...
mysql_query("UPDATE `ngocrong_nuituyet` SET `hp`=`hp`-'".$dameboss."' WHERE `id`='$id'");
