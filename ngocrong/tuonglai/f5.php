...
while($s=mysql_fetch_array($e)) {
    ...
    if (time()>=$s['time'] && $s['hp']==0){

        $randboss=rand(0,6);
                    $tuonglai = mysql_fetch_array(mysql_query("SELECT * FROM `ngocrong_tuonglai` WHERE `id` = '{$randboss}' "));

        if ($randboss==6){
            $text = 'Xên hoàn thiện vừa xuất hiện ở Khu '.$s['id'].' Tương lai  ';
...
");
       ...
       mysql_query("UPDATE `ngocrong_tuonglai_khu` SET `boss` ='".$randboss."', `time`='".time()."',`hp`= '".$tuonglai['hpfull']."' WHERE `id` = '".$s['id']."'");
