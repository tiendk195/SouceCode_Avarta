...
$wboss = mysql_fetch_array(mysql_query("SELECT * FROM `ngocrong_cold` WHERE `id` = '$id'"));
 ...
 if ($ad['sudung']==1) {
    $bossdame=0;
 	...
 	if ($datauser['matna']==258 ){
 	 	...
 	 	if ($datauser['matna']==256 ){
 	     $dameboss=$dameboss+($dameboss*50/100);
 	     
 	}
 		 	if ($datauser['matna']==257 ){
 	     $dameboss=$dameboss+($dameboss*60/100);
 	...
 	if ($datauser['matna']==255 ){
 		...
 		if ($datauser['matna']!=258 or $datauser['matna']!=256 or $datauser['matna']!=252 or $datauser['matna']!=253 or $datauser['matna']!=255 or $datauser['matna']!=257   ){
...
$timeboss=$wboss['time']-time();

if(time() < $wboss['time']){
echo'<script>alert("'.$wboss['tenboss'].' sẽ xuất hiện trong '.$timeboss.'s nữa!");</script>';

}else if ($wboss['hp']<=0){
   mysql_query("UPDATE `ngocrong_cold` SET `hp`=`hpfull` WHERE `id`='$id'");
...
} else if($wboss['hp'] <= $dameboss){

if ($dameboss >= $wboss['hp']) {
        $randcs=rand(1,10);
    $csb=rand(1,3);
    $csv=rand(1,1);
    if ($randcs==1){
...
mysql_query("UPDATE `ngocrong_cold` SET `hp`='0' , `time` = {$timeboss} WHERE `id`='$id'");
...
");
if ($wboss['id']==5){
    			                  		    echo'<script>alert("Bạn nhận được 1 Rương ngọc!! ");</script>';
...
mysql_query("UPDATE `ngocrong_cold` SET `hp`=`hp`-'".$dameboss."' WHERE `id`='$id'");
