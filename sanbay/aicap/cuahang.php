...
echo'<div class="phdr">Shop Ai Cập</div>';
...
switch($act) {
default:
$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `shopaicap` "),0);
$req=mysql_query("SELECT * FROM `shopaicap` ORDER BY `id` DESC LIMIT $start,$kmess");
...
$id=$_GET[id];
$item=mysql_fetch_array(mysql_query("SELECT * FROM `shopaicap` WHERE `id`='".$id."'"));
$shop=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$item[idshop]."'"));
  $xyz=mysql_num_rows(mysql_query("SELECT * FROM `shopaicap` WHERE `id`='".$id."'"));
...
@mysql_query("UPDATE `shopaicap` SET
`luong`='".$_POST[luong]."',
...
mysql_query("DELETE FROM `shopaicap` WHERE `id`='".$id."'");
    header ('Location : cuahang.php ');

}
$checkit = mysql_result(mysql_query("select count(*) from `shopaicap` where `id` = '".$id."'"),0);
if ($checkit == 0) {
...
mysql_query("INSERT INTO `shopaicap` SET
`idshop`='".$vatpham."',
`luong`='".$luong."',
`timebd`='".$hsd."'");
$tenvatpham=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$vatpham."'"));
$bot='[b]'.$login.' vừa thêm [red]'.$tenvatpham[tenvatpham].' [/red] vào shop ai cập đặc biệt![/b]';
...
$id = (int)$_GET['id'];
$check=mysql_num_rows(mysql_query("SELECT * FROM `shopaicap` WHERE `id`='".$id."'"));
if ($check<1) {
header('Location: cuahang.php');
exit;
}
