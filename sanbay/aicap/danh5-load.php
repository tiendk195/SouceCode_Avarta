...
<?php
//-- Online Topic ---//
mysql_query("UPDATE `users` SET `vitri`='672' WHERE `id`='{$user_id}'");

$online_u = mysql_result(mysql_query("SELECT COUNT(*) FROM `users` WHERE `lastdate` > " . (time() - 300) . " AND `vitri` = '672'"), 0);
$totalonline = $online_u;
$q = @mysql_query("select * from `users` where `lastdate` > " . (time() - 300) . " AND `vitri` = '672';");
