...
$hh=mysql_fetch_array(mysql_query("SELECT * FROM `vatpham` WHERE `user_id`='".$user_id."' AND `id_shop`='166'"));
...
switch($act) {
default:
echo'<div class="omenu"><center>Bạn đang có <font color="red">'.$hh['soluong'].'</font> Huy hiệu huyền thoại <img src="/images/vatpham/166.png"></center></div>';

$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `doiquavqbd` "),0);
$req=mysql_query("SELECT * FROM `doiquavqbd` ORDER BY `id` DESC LIMIT $start,$kmess");
...
$id=$_GET[id];
$item=mysql_fetch_array(mysql_query("SELECT * FROM `doiquavqbd` WHERE `id`='".$id."'"));
$shop=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$item[idshop]."'"));
  $xyz=mysql_num_rows(mysql_query("SELECT * FROM `doiquavqbd` WHERE `id`='".$id."'"));
...
@mysql_query("UPDATE `doiquavqbd` SET
`diem`='".$_POST[diem]."',
...
mysql_query("DELETE FROM `doiquavqbd` WHERE `id`='".$id."'");
    header ('Location : doiqua.php ');

}
$checkit = mysql_result(mysql_query("select count(*) from `doiquavqbd` where `id` = '".$id."'"),0);
if ($checkit == 0) {
...
mysql_query("INSERT INTO `doiquavqbd` SET
`idshop`='".$vatpham."',
`diem`='".$diem."',
`timebd`='".$hsd."'");
...
$id = (int)$_GET['id'];
$check=mysql_num_rows(mysql_query("SELECT * FROM `doiquavqbd` WHERE `id`='".$id."'"));
if ($check<1) {
header('Location: doiqua.php');
exit;
}
...
mysql_query("UPDATE `vatpham` SET `soluong`=`soluong`-'".$item['diem']."' WHERE `user_id`='".$user_id."' AND `id_shop`='166'");
