...
$hh=mysql_fetch_array(mysql_query("SELECT * FROM `vatpham` WHERE `user_id`='".$user_id."' AND `id_shop`='167'"));

echo'<div class="phdr">Dr Doom &gt; Đổi quà đặc biệt &gt; </div>';
...
switch($act) {
default:
echo'<div class="omenu"><center>Bạn đang có <font color="red">'.$hh['soluong'].'</font> Huy hiệu hoàng gia <img src="/images/vatpham/167.png"></center></div>';

$tong =mysql_result(mysql_query("SELECT COUNT(*) FROM `doiquavqbd1` "),0);
$req=mysql_query("SELECT * FROM `doiquavqbd1` ORDER BY `id` DESC LIMIT $start,$kmess");
...
<td class="right-info">
<b><font color="blue">'.$shop[tenvatpham].'</font></b>';
if ($res['timebd']==0) { 
              echo' (Vĩnh viễn)'; 
              } else {
...
echo'<br>
Cần: <font color="red">'.number_format($res['diem']).'</font> Huy hiệu huyền thoại<br>
...
$id=$_GET[id];
$item=mysql_fetch_array(mysql_query("SELECT * FROM `doiquavqbd1` WHERE `id`='".$id."'"));
$shop=mysql_fetch_array(mysql_query("SELECT * FROM `shopdo` WHERE `id`='".$item[idshop]."'"));
  $xyz=mysql_num_rows(mysql_query("SELECT * FROM `doiquavqbd1` WHERE `id`='".$id."'"));
...
@mysql_query("UPDATE `doiquavqbd1` SET
`diem`='".$_POST[diem]."',
...
echo 'Nhập huy hiệu: <input type="number" name="luong" value="'.$item[diem].'"><br/>';
...
mysql_query("DELETE FROM `doiquavqbd1` WHERE `id`='".$id."'");
    header ('Location : doiqua1.php ');

}
$checkit = mysql_result(mysql_query("select count(*) from `doiquavqbd1` where `id` = '".$id."'"),0);
if ($checkit == 0) {
...
*/
if (empty($vatpham) or empty($diem) ) {
echo '<div class="news">Không được bỏ trống chỗ nào hết!</div>';
} else {
mysql_query("INSERT INTO `doiquavqbd1` SET
`idshop`='".$vatpham."',
`diem`='".$diem."',
`timebd`='".$hsd."'");
...
echo '</select><br/>';
echo 'Cần huy hiệu: <input type="number" name="diem" size="5"><br/>';
...
$id = (int)$_GET['id'];
$check=mysql_num_rows(mysql_query("SELECT * FROM `doiquavqbd1` WHERE `id`='".$id."'"));
if ($check<1) {
...
echo'</font> với <b>'.number_format($item['diem']).' huy hiệu không </b>? <br>
...
if($hh['soluong'] >= $item['diem']){
    
    if ($item['timebd'] !=0)
{
$timesd= $item['timebd']*24*3600+time();
}
mysql_query("UPDATE `vatpham` SET `soluong`=`soluong`-'".$item['diem']."' WHERE `user_id`='".$user_id."' AND `id_shop`='167'");
