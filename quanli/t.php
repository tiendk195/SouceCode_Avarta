...
if($loai){
		...
		$total2 =mysql_result(mysql_query("SELECT COUNT(*) FROM `shopdo` WHERE `hienthi`='1'"), 0);
...
if ($datauser['rights']>=9) {
    echo'<div class="omenu"><a href="iteman.php?act=lichsu">Xem lịch sử tặng vật phẩm</a></div>';
...
$req=mysql_query("SELECT * FROM `shopdo` WHERE `hienthi`='1' AND `id`!='0' AND `loai`= '".$loai."' ORDER BY `id` DESC LIMIT $start,$kmess");
}
...
<font color="ff007f">Vật phẩm:</font>   '.$res[tenvatpham].'  </font></br>
		  ...
		  if ($res['nguoiup']>0) {
		      ...
		      $nguoiup=mysql_fetch_array(mysql_query("SELECT * FROM `users` WHERE `id`='".$res['nguoiup']."'"));
...
$total =mysql_result(mysql_query("SELECT COUNT(*) FROM `shopdo` WHERE `hienthi`='1' AND `loai`='".$loai."'"), 0);
}
if($total == 0){
